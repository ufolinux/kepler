self.description = "kepler-db-upgrade DB version 9 (symlink support removal)"
...
self.cmd = ["kepler-db-upgrade",
        "--root", self.rootdir(),
        "--dbpath", self.dbdir(),
        "--config", self.configfile() ]

self.addrule("KEPLER_RETCODE=0")
self.addrule("FILE_EXIST=var/lib/kepler/local/ALPM_DB_VERSION")
