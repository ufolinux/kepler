...
class pmtest(object):
    ...
    def generate(self, kepler):
        ...
        ldconfig = os.path.basename(kepler["ldconfig"])
        ldconfigdir = os.path.join(self.root, os.path.dirname(kepler["ldconfig"][1:]))
        shell = kepler["scriptlet-shell"][1:]
    ...
    def add_hook(self, name, content):
        if not name.endswith(".hook"):
            name = name + ".hook"
        path = os.path.join("etc/kepler.d/hooks/", name)
    ...
    def run(self, kepler):
        if os.path.isfile(util.PM_LOCK):
            tap.bail("\tERROR: another kepler session is on-going -- skipping")
            return

        tap.diag("==> Running test")
        vprint("\tkepler %s" % self.args)
        ...
        prog = kepler["bin"]
        if not prog:
            prog = util.which(self.cmd[0], kepler["bindir"])
        ...
        if kepler["manual-confirm"]:
            cmd.append("--confirm")
        if kepler["debug"]:
            cmd.append("--debug=%s" % kepler["debug"])
        cmd.extend(shlex.split(self.args))

        if not (kepler["gdb"] or kepler["nolog"]):
        ...
        # Change to the tmp dir before running kepler, so that local package
        ...
        if os.path.isfile(os.path.join(self.root, util.TMPDIR, "core")):
            tap.diag("\tERROR: kepler dumped a core file")
