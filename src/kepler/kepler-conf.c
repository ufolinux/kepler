/*
 *  kepler-conf.c - parse kepler configuration files
 *
 *  Copyright (c) 2013-2022 Kepler Development Team <kepler-dev@lists.archlinux.org>
 *
...
const char *myname = "kepler-conf", *myver = "1.0.0";
...
static void usage(int ret)
{
	...
	fputs(_("kepler-conf - query kepler's configuration file\n"), stream);
	fputs(_("usage:  kepler-conf [options] [<directive>...]\n"), stream);
	fputs(_("        kepler-conf (--repo-list|--help|--version)\n"), stream);
	fputs(_("options:\n"), stream);
