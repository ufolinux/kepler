...
/** Output kepler version and copyright.
 */
static void version(void)
{
	printf("\n");
	printf(" .--.                  Kepler v%s - libalpm v%s\n", PACKAGE_VERSION, alpm_version());
	printf("/ _.-' .-.  .-.  .-.   Copyright (C) 2006-2022 Kepler Development Team\n");
...
static void setuseragent(void)
{
	...
	uname(&un);
	len = snprintf(agent, 100, "kepler/%s (%s %s) libalpm/%s",
...
static void cl_to_log(int argc, char *argv[])
{
	...
	if(cl_text) {
		alpm_logaction(config->handle, KEPLER_CALLER_PREFIX,
				"Running '%s'\n", cl_text);
		free(cl_text);
	}
}
...
int main(int argc, char *argv[])
{
	...
	pm_printf(ALPM_LOG_DEBUG, "kepler v%s - libalpm v%s\n", PACKAGE_VERSION, alpm_version());
	...
	switch(config->op) {
		case PM_OP_DATABASE:
			ret = kepler_database(pm_targets);
			break;
		case PM_OP_REMOVE:
			ret = kepler_remove(pm_targets);
			break;
		case PM_OP_UPGRADE:
			ret = kepler_upgrade(pm_targets);
			break;
		case PM_OP_QUERY:
			ret = kepler_query(pm_targets);
			break;
		case PM_OP_SYNC:
			ret = kepler_sync(pm_targets);
			break;
		case PM_OP_DEPTEST:
			ret = kepler_deptest(pm_targets);
			break;
		case PM_OP_FILES:
			ret = kepler_files(pm_targets);
			break;
		default:
