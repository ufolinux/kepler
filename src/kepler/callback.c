...
static struct kepler_progress_bar *totalbar;
...
struct kepler_multibar_ui {
	 ...
	 */
	alpm_list_t *active_downloads; /* List of type 'struct kepler_progress_bar' */
...
struct kepler_multibar_ui multibar_ui = {0};

static int dload_progressbar_enabled(void);
static void init_total_progressbar(void);
static void update_bar_finalstats(struct kepler_progress_bar *bar);
static void draw_kepler_progress_bar(struct kepler_progress_bar *bar);
	...
	switch(event->type) {
		...
		case ALPM_EVENT_PKG_RETRIEVE_FAILED:
			...
			if(total_enabled) {
				...
				draw_kepler_progress_bar(totalbar);
				free(totalbar);
				printf("\n");
			}
			total_enabled = 0;
...
static bool find_bar_for_filename(const char *filename, int *index, struct kepler_progress_bar **bar)
{
	int i = 0;
	alpm_list_t *listitem = multibar_ui.active_downloads;
	for(; listitem; listitem = listitem->next, i++) {
		struct kepler_progress_bar *b = listitem->data;
...
static void init_total_progressbar(void)
{
	totalbar = calloc(1, sizeof(struct kepler_progress_bar));
	assert(totalbar);
...
static void dload_init_event(const char *filename, alpm_download_event_init_t *data)
{
	...
	struct kepler_progress_bar *bar = calloc(1, sizeof(struct kepler_progress_bar));
	assert(bar);
 ...
 */
static bool update_bar_stats(struct kepler_progress_bar *bar)
{
	int64_t timediff;
...
{
	int index;
	struct kepler_progress_bar *bar;
	bool ok;
	off_t last_chunk_amount;
...
static void dload_complete_event(const char *filename, alpm_download_event_completed_t *data)
{
	...
	if(data->result == 1) {
		...
		if(multibar_ui.move_completed_up && index != 0) {
			 ...
			 */
			struct kepler_progress_bar *former_topbar = multibar_ui.active_downloads->data;
			...
			console_cursor_goto_bar(index);
			draw_kepler_progress_bar(former_topbar);
	...
	while(multibar_ui.active_downloads) {
		...
		struct kepler_progress_bar *j = head->data;
		if(j->completed) {
			multibar_ui.cursor_lineno--;
			multibar_ui.active_downloads_num--;
