...
Name
----
kepler.conf - kepler package manager configuration file


Synopsis
--------
{sysconfdir}/kepler.conf


Description
-----------
Kepler, using linkman:libalpm[3], will attempt to read kepler.conf each time it
is invoked. This configuration file is divided into sections or repositories.
Each section defines a package repository that kepler can use when searching
...
*RootDir =* /path/to/root/dir::
	Set the default root directory for kepler to install to. This option is
...
*DBPath =* /path/to/db/dir::
	...
	default is +{localstatedir}/lib/kepler/+. Most users will not need to set
	this option. *NOTE*: if specified, this is an absolute path and the root
	path is not automatically prepended.

*CacheDir =* /path/to/cache/dir::
	Overrides the default location of the package cache directory. The
	default is +{localstatedir}/cache/kepler/pkg/+. Multiple cache directories can be
...
*HookDir =* /path/to/hook/dir::
	...
	+{sysconfdir}/kepler.d/hooks+.  Multiple directories can be specified with
...
*GPGDir =* /path/to/gpg/dir::
	...
	files for GnuPG. The default is +{sysconfdir}/kepler.d/gnupg/+.
...
*LogFile =* /path/to/log/file::
	Overrides the default location of the kepler log file. The default
	is +{localstatedir}/log/kepler.log+. This is an absolute path and the root directory
	is not prepended.

*HoldPkg =* package ...::
	If a user tries to '\--remove' a package that's listed in `HoldPkg`,
	kepler will ask for confirmation before proceeding. Shell-style glob
	patterns are allowed.

*IgnorePkg =* package ...::
	Instructs kepler to ignore any upgrades for this package when performing
	a '\--sysupgrade'. Shell-style glob patterns are allowed.

*IgnoreGroup =* group ...::
	Instructs kepler to ignore any upgrades for all packages in this
...
*Architecture =* auto &| i686 &| x86_64 | ...::
	If set, kepler will only allow installation of packages with the given
...
*Color*::
	Automatically enable colors only when kepler's output is on a tty.
...
Include = {sysconfdir}/kepler.d/mirrorlist
--------
...
+
During parsing, kepler will define the `$repo` variable to the name of the
current section. This is often utilized in files specified using the 'Include'
directive so all repositories can use the same mirrorfile. kepler also defines
...
directory with these packages so kepler can find it when run with '\--refresh'.
...
in the configuration file as shown in the config example above. Kepler will now
use your package repository. If you add new packages to the repository,
remember to re-generate the database and use kepler's '\--refresh' option.
...
See Also
--------
linkman:kepler[8], linkman:libalpm[3]
