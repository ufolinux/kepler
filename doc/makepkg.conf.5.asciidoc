...
Synopsis
--------
{sysconfdir}/makepkg.conf, $XDG_CONFIG_HOME/kepler/makepkg.conf, ~/.makepkg.conf
...
$XDG_CONFIG_HOME/kepler/makepkg.conf or ~/.makepkg.conf, with the former
taking priority.
...
**KEPLER_AUTH=()**::
	Specify a command prefix for running kepler as root. If unset, makepkg will
...
See Also
--------
linkman:makepkg[8], linkman:kepler[8], linkman:PKGBUILD[5]
