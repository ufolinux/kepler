...
Name
----
kepler - package manager utility


Synopsis
--------
'kepler' <operation> [options] [targets]


Description
-----------
Kepler is a package management utility that tracks installed packages on a Linux
...
automatically upgrade packages. Kepler packages are a zipped tar format.

Since version 3.0.0, kepler has been the front-end to linkman:libalpm[3], the
...
Invoking kepler involves specifying an operation with any potential options and
...
*-D, \--database*::
	...
	certain attributes of the installed packages in kepler's database. It
...
*-S, \--sync*::
	...
	example, `kepler -S qt` will download and install qt and all the
	...
	package to install: `kepler -S testing/qt`. You can also specify version
	requirements: `kepler -S "bash>=3.2"`. Quotes are needed, otherwise the
	shell interprets ">" as redirection to a file.
+
In addition to packages, groups can be specified as well. For example, if
gnome is a defined package group, then `kepler -S gnome` will provide a
...
+
Packages that provide other packages are also handled. For example, `kepler -S
...
+
You can also use `kepler -Su` to upgrade all packages that are out-of-date. See
<<SO,Sync Options>> below. When upgrading, kepler performs version comparison
...
*-T, \--deptest*::
	...
	This operation accepts no other options. Example usage: `kepler -T qt
	"bash>=3.2"`.
...
*-U, \--upgrade*::
	...
	on how kepler takes care of configuration files.
...
*-b, \--dbpath* <path>::
	...
	+{localstatedir}/lib/kepler+). This should not be used unless you know what
	you are doing.
...
*-r, \--root* <path>::
	...
	*NOTE*: If database path or log file are not specified on either the
	command line or in linkman:kepler.conf[5], their default location will
...
*\--cachedir* <dir>::
	...
	+{localstatedir}/cache/kepler/pkg+). Multiple cache directories can be
	specified, and they are tried in the order they are passed to kepler.
...
*\--gpgdir* <dir>::
	...
	(the default is +{sysconfdir}/kepler.d/gnupg+). This directory should contain
...
*\--hookdir* <dir>::
	...
	+{sysconfdir}/kepler.d/hooks+). Multiple hook directories can be specified
...
*\--noconfirm*::
	...
	this unless you want to run kepler from a script.
...
*\--sysroot* <dir>::
	Specify an alternative system root.  Kepler will chroot and chdir into the
...
*-d, \--nodeps*::
	...
	kepler will always check a package's dependency fields to ensure that all
...
*\--noprogressbar*::
	...
	for scripts that call kepler and capture the output.
...
*\--ignore* <package>::
	Directs kepler to ignore upgrades of package even if there is one
	available. Multiple packages can be specified by separating them
	with a comma.

*\--ignoregroup* <group>::
	Directs kepler to ignore upgrades of all packages in 'group', even if
...
*-q, \--quiet*::
	...
	kepler's output is processed in a script. Search will only show package
...
*-n, \--nosave*::
	Instructs kepler to ignore file backup designations. Normally, when a
...
*-c, \--clean*::
	...
	currently unused sync databases to free up disk space. When kepler
	...
	linkman:kepler.conf[5]. Use one '\--clean' switch to only remove
...
*-u, \--sysupgrade*::
	...
	necessary.
+
Pass this option twice to enable package downgrades; in this case, kepler will
...
*-y, \--refresh*::
	...
	defined in linkman:kepler.conf[5]. This should typically be used each time
...
*-q, \--quiet*::
	...
	kepler's output is processed in a script, however, you may want to use
...
----------------------------
Kepler uses the same logic as 'rpm' to determine action against files that are
...
kepler -U /home/user/ceofhack-0.6-1-x86_64.pkg.tar.gz::
...
Configuration
-------------
See linkman:kepler.conf[5] for more details on configuring kepler using the
'kepler.conf' file.
