...
packages built with linkman:makepkg[8] and installed with linkman:kepler[8].
...
kepler and a large database containing package file lists ``foo.files.tar.xz'' for
use by other utilities. While kepler can use the large database (if renamed with a
db.tar* extension), there is currently no additional benefit for the larger download.


See Also
--------
linkman:makepkg[8], linkman:kepler[8]
