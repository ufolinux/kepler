...
This document is here to guide you in helping translate kepler messages,
libalpm messages, and the manual pages for the entire kepler package.

We are currently using https://www.transifex.com/[Transifex] as the translation
platform for kepler and libalpm. You will need to sign up for an account there
and then register with a translation team on the
https://www.transifex.com/projects/p/archlinux-kepler/[kepler project page].
...
There are two separate message catalogs in kepler: one for the back-end
(libalpm) and one for the front-end (kepler and scripts). These correspond to
the `lib/libalpm/po` and `po` directories in the kepler source, respectively.
...
Or to just push one of the two available resources:
	...
	tx push -r archlinux-kepler.kepler-pot -t -l <mylang>
	tx push -r archlinux-kepler.libalpm-pot -t -l <mylang>
...
and an email will be sent to the mailto:kepler-dev@lists.archlinux.org[kepler-dev]
...
If you have more advanced needs you will have to get a copy of the kepler
repository.
...
`po/` tree, replace 'libalpm.pot' with 'kepler.pot':
...
substituting 'libalpm.pot' or 'kepler.pot' for potfile depending on which
...
If you want to test the translation (for example, the front-end one):
	...
	rm *.gmo stamp-po
	make
	cp <lang code>.gmo /usr/share/locale/<lang code>/LC_MESSAGES/kepler.mo


Translating Manpages
--------------------
There are currently no efforts underway to include translated manual pages in
the kepler codebase. However, this is not to say translations are unwelcome. If
someone has experience with i18n man pages and how to best include them with our
source, please contact the kepler-dev mailing list at
mailto:kepler-dev@lists.archlinux.org[].
