...
*epoch*::
	...
	version comparison logic. See linkman:kepler[8] for more information on
	version comparisons.
...
*backup (array)*::
	...
	`"Handling Config Files"` in linkman:kepler[8] for more information.
...
*optdepends (array)*::
	...
	and are not utilized by kepler during dependency resolution. Packages in
...
+
Sysupgrade is currently the only kepler operation that utilizes this field.
...
--------------------------------
Kepler has the ability to store and execute a package-specific script when it
...
See Also
--------
linkman:makepkg[8], linkman:kepler[8], linkman:makepkg.conf[5]
